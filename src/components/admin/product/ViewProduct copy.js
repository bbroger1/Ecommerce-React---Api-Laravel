...
function ViewProduct() {
    ...
    const columns = ["id", "image", "name", "category", "price", "status"];
    ...
    const handleSort = (column) => {
        if (column === sortColumn) {
            sortOrder === sortOrder ? setSortOrder(SORT_DESC) : setSortOrder(SORT_ASC)
        } else {
    ...
    useEffect(() => {
        ...
        axios.post(`/api/view-product`)
            .then(res => {
                if (res.status === 200) {
                    setProductList(res.data.products.data);
                }
                setLoading(false);
            })
            .catch(res => {
